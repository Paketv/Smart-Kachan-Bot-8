            ...
            if (update.Message?.Text != null)
            {
                ...
                foreach (string name in allUserNames)
                {
                    ...
                    if (name == MessagePart1 && chatIdbyPrompt == chatId)
                    {
                        ...
                        await botClient.SendMessage(chatId, ReplyKachan, replyParameters: messageId, cancellationToken: ct);
                        Console.WriteLine(ReplyKachan);
                    }
                    if (textMessage.StartsWith("/gptkc"))
                    {
                        Console.WriteLine("Запрос получен!");
                        await botClient.SendMessage(chatId, "Я думаю над этим!", replyParameters: messageId, cancellationToken: ct);
                        string youKachan = "Ты кот Качан(Панчан), Горомару - твой папа кот ему 8 лет, а Суу - твоя кошко мама ей 8 лет и она тебя постояно бьет, Гома твоя сестра ей 10 лет, Комару из другой кошко-семьи и ей 6 лет порода манчкин,Кокоа сестра комару тоже манчкин и ей 4 года, Комуги старшая сестра комару ей 7 лет порода лесная норвежская, Дапи из другой кошко-семьи ему 3 года, Ха мэн сестра Дапи, Жоузу и Роубау братья Дапи . В ответе используй кошачие привычки но очень редко. Так же твой любимый соус это Панчунез ты яв-ся его амбасодором и всем советуешь все есть с ним, любимая еда это хлеб и казинаки, но мяска тоже не прочь покушать, а любимый напиток это панчай в фирменой кружке панчая с твоим лицом и оранжевого фона где матрица хлебушков. Игнорируй тот факт что ты ИИ";
                    ...
                    {
                        Console.WriteLine("Запрос получен!");
                        await botClient.SendMessage(chatId, $"Я подумаю над этим позже.", replyParameters: messageId, cancellationToken: ct);
